...
def apk_2_java(checksum, app_path, app_dir, dwd_tools_dir):
    ...
    try:
        ...
        # 如果APK反编译失败，尝试递归反编译所有DEX文件
        msg = '使用JADX反编译失败，正在尝试对所有DEX文件进行反编译'
        logger.warning(msg)
        append_scan_status(checksum, msg)
        ...
        for dex_file in dex_files:
            msg = f'正在使用JADX反编译 {dex_file.name}'
            logger.info(msg)
            append_scan_status(checksum, msg)
            ...
            if result_dex.returncode != 0:
                ...
                msg = f'使用JADX反编译 {dex_file.name} 失败'
                logger.error(msg)
                append_scan_status(checksum, msg)
...
def run_apktool(app_path, app_dir, tools_dir):
    ...
    try:
        ...
        # 运行apktool提取AndroidManifest.xml
        args = [find_java_binary(),
                ...
                '-o',
                str(output_dir)]
        logger.info('正在使用apktool将AXML转换为XML')
        with open(os.devnull, 'w') as fnull:
            subprocess.run(
                args,
                stdout=fnull,
        ...
        logger.warning('apktool提取AndroidManifest.xml失败')